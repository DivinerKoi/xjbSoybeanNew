...
import { menuIconTypeOptions, specialEnableStatusOptions } from '@/constants/business';
import SvgIcon from '@/components/custom/svg-icon.vue';
import { getLocalIcons } from '@/utils/icon';
import { fetchAddMenu, fetchUpdateMenu } from '@/service/api/system/menu';
import {
  getLayoutAndPage,
...
const model: Api.SystemManage.Menu = reactive(createDefaultModel());
...
type RuleKey = Extract<keyof Model, 'menuName' | 'status' | 'routeName' | 'routePath' | 'path'>;
...
const showPage = computed(() => model.menuType === 'C');

const showButton = computed(() => model.menuType === 'F');
...
// 菜单类型选项
const menuTypeOptions: CommonType.Option[] = [
  { label: '目录', value: 'M' },
  { label: '菜单', value: 'C' },
  { label: '按钮', value: 'F' }
];
...
function handleInitModel() {
  ...
  if (props.operateType === 'add') {
    // if (props.rowData?.menuId) {
    //   const { menuId } = props.rowData;
    //   Object.assign(model, { parentId: menuId, menuId: '' });
    // } else {
    //   model.parentId = 0;
    // }
  }
  ...
  if (props.operateType === 'edit') {
    ...
    const { path, param } = getPathParamFromRoutePath(rest.path as string);
    ...
    <NScrollbar class="h-480px pr-20px">
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100">
        <NGrid responsive="screen" item-responsive>
          ...
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.routeName')" path="routeName">
            <NInput v-model:value="model.routeName" :placeholder="$t('page.manage.menu.form.routeName')" />
          </NFormItemGi>
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.routePath')" path="path">
            <NInput v-model:value="model.path" disabled :placeholder="$t('page.manage.menu.form.path')" />
          </NFormItemGi>
          <!--   pathParam、 layout和page 不参与提交，只用于构造component      -->
          <NFormItemGi
          ...
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.iconTypeTitle')" path="iconType">
          ...
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.icon')" path="icon">
          ...
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.menuStatus')" path="status">
          ...
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.keepAlive')" path="keepAlive">
          ...
          <!--            </NRadioGroup>-->
          <!--          </NFormItemGi>-->
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.href')" path="href">
            <NInput v-model:value="model.href" :placeholder="$t('page.manage.menu.form.href')" />
          </NFormItemGi>
          <NFormItemGi v-if="!showButton" span="24 m:12" :label="$t('page.manage.menu.hideInMenu')" path="hideInMenu">
